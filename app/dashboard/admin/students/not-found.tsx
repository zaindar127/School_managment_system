...
export function NotFound({
  ...
  return (
    <div className="relative text-center z-[1] pt-20 md:pt-32">
      <h1 className="mt-4 text-balance text-5xl font-semibold tracking-tight text-primary sm:text-7xl">
        {title}
      </h1>
      <p className="mt-6 text-pretty text-lg font-medium text-muted-foreground sm:text-xl/8 max-w-xl mx-auto">
        {description}
      </p>
      <div className="mt-10 flex flex-col sm:flex-row sm:items-center sm:justify-center gap-y-3 gap-x-6">
        ...
        <Button className="-order-1 sm:order-none" asChild>
          <Link href="/students">Take me home</Link>
        </Button>
      </div>
    </div>
  )
  ...
  return (
    <div className="relative flex flex-col w-full justify-center min-h-svh bg-background p-6 md:p-10">
      <div className="relative max-w-5xl mx-auto w-full">
        ...
        <NotFound
          title="Page not found"
          description="We couldn't find the page you're looking for. Please check the URL or return to the homepage."
        />
      </div>
    </div>
  )
